$(document).ready(function () {
    ...
    function initVisibleCharts() {
        // Initialize employee skill charts
        ...
        // Initialize process min_skill charts
        $('canvas.process-chart').each(function () {
            const canvas = $(this);
            const chartId = canvas.attr('id');
            const minSkill = parseInt(canvas.data('min-skill')) || 0;
            const maxSkill = 4;

            // Avoid re-initializing the same chart
            if (!canvas.data('initialized')) {
                generateChart(chartId, minSkill, maxSkill);
                canvas.data('initialized', true);
            }
        });
    }
