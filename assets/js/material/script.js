        ...
        function saveData() {
            ...
            const requiredFields = ['tanggal', 'shift', 'no_material', 'nama_material', 'kode', 'sebelum', 'saatini'];
            let isValid = true;
            ...
            // Send data to server
            fetch('../proses/material/create.php', {
                method: 'POST',
                body: formData
            })
            ...
            resetForm();
            bootstrap.Modal.getInstance(document.getElementById('addModalMM')).hide();
        }

        function editData(id) {
            editIndex = id;
            fetch(`../proses/material/get_data.php?id=${id}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('no_material').value = data.data.no_material;
                        document.getElementById('nama_material').value = data.data.nama_material;
                        ...
                        const modal = new bootstrap.Modal(document.getElementById('addModalMM'));
                        modal.show();
                    } else {
                        Swal.fire({
                            icon: 'error',
        ...
        function deleteData(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`../proses/material/delete_data.php`, {
                        method: 'POST',
                        body: formData
                    })
                    .then(response => response.json())
        ...
        document.getElementById('addModalMM').addEventListener('hidden.bs.modal', function () {
            resetForm();
        });
